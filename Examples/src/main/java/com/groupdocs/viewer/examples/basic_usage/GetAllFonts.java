...
import com.groupdocs.viewer.fonts.IFontInfo;
...
public class GetAllFonts {
    public static void run() {
        try (Viewer viewer = new Viewer(TestFiles.SAMPLE_DOCX)) {
            IFontInfo[] list = viewer.getAllFonts();
            for (IFontInfo font : list) {
                System.out.printf(
                        "Font '%s' of %s style has %d bytes and is of %s format%n",
                        font.getFamilyName(),
                        font.getStyle().getName(),
